...
class SupervisorNamespaceRPCInterface:
    ...
    def startProcess(self, name, wait=True):
        ...
        if wait and process.get_state() != ProcessStates.RUNNING:
            ...
            def onwait():
                ...
                if state == ProcessStates.STOPPING and process.config.stopnowait:
                    raise RPCError(Faults.WAITING_TO_START, name)
    ...
    def stopProcess(self, name, wait=True):
        ...
        if process.config.stopnowait:
            self.supervisord.options.logger.debug('stopnowait: %d' % process.config.stopnowait)
            return True
    ...
    def markRestartAllProcesses(self):
        """ Mark all processes need restart after stop
        """
        self.supervisord.options.logger.info('mark need_restart for all processes')

        processes = self._getAllProcesses()
        markAll = make_allfunc(processes, isRunning, self.markRestartForProcess)

        markAll.delay = 0.05
        markAll.rpcinterface = self
        return markAll  # deferred

    def markRestartProcessInGroup(self, name):
        """ Mark all processes need restart in the process group named 'name'
        ...
        self.supervisord.options.logger.info('mark need_restart for group: "%s"' % name)
        for process in list(group.processes.values()):
            process.needRestart()
        return True

    def markRestartForProcess(self, name):
        """ Mark a process as need_restart
        ...
        if process is None:
            group_name, process_name = split_namespec(name)
            return self.markRestartProcessInGroup(group_name)

        self.supervisord.options.logger.info('mark need_restart to process: "%s"' % name)
        process.needRestart()
        return True
